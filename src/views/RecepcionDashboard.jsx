...
export default function RecepcionDashboard({ user, setUser }) {
  ...
  const [pacientesSala, setPacientesSala] = useState([
    { id: 1, nombre: 'Juan Pérez', documento: '12345678', servicio: 'Consulta Médica General', estado: 'EN_ESPERA', horaIngreso: '08:15 AM' },
    { id: 2, nombre: 'María Gómez', documento: '87654321', servicio: 'Certificado de Huella', estado: 'EN_ESPERA', horaIngreso: '08:30 AM' },
    { id: 3, nombre: 'Carlos Ruiz', documento: '45678912', servicio: 'Crecimiento y Desarrollo', estado: 'ATENDIDO', horaIngreso: '07:50 AM' }
  ]);
    ...
    <div className="flex h-screen bg-gray-100 font-sans">
      ...
      <aside className="w-64 bg-[#1b75bb] text-white flex flex-col justify-between shadow-xl z-10">
          ...
          <nav className="mt-4 px-2 space-y-1">
            <button
              ...
              className={`w-full text-left px-4 py-3 rounded-lg text-xs font-bold transition-all ${
                seccionActiva === 'admision' ? 'bg-[#00adee] text-white shadow-md' : 'hover:bg-blue-600 text-blue-100'
              }`}
            >
            ...
            <button
              ...
              className={`w-full text-left px-4 py-3 rounded-lg text-xs font-bold transition-all ${
                seccionActiva === 'facturacion' ? 'bg-[#00adee] text-white shadow-md' : 'hover:bg-blue-600 text-blue-100'
              }`}
            >
            ...
            <button
              ...
              className={`w-full text-left px-4 py-3 rounded-lg text-xs font-bold transition-all ${
                seccionActiva === 'control-tv' ? 'bg-[#00adee] text-white shadow-md' : 'hover:bg-blue-600 text-blue-100'
              }`}
            >
        ...
        <div className="p-4 border-t border-blue-400 bg-blue-900/20">
          <p className="text-xs font-bold truncate">{user?.nombreCompleto || 'Auxiliar Administrativo'}</p>
          <p className="text-[10px] text-blue-200 uppercase tracking-wider font-semibold">Recepción</p>
          <div className="mt-2 flex items-center space-x-2">
            <span className={`h-2 w-2 rounded-full ${conectado ? 'bg-green-400 animate-pulse' : 'bg-red-500'}`}></span>
            <span className="text-[10px] text-blue-100 font-medium">{conectado ? 'WS Conectado' : 'WS Desconectado'}</span>
          </div>
          <button
      ...
      <main className="flex-1 flex flex-col overflow-hidden bg-gray-50">
        ...
        {/* VISTA 1: ADMISIÓN DE PACIENTES (División 65% App / 35% Rentarhosting) */}
        {seccionActiva === 'admision' && (
          <div className="flex-1 flex p-3 gap-3 overflow-hidden">
            
            {/* Lado Izquierdo (65%): Formulario + Matriz del Día en nuestra App */}
                ...
                <form onSubmit={handleRegistrarTurno} className="grid grid-cols-2 gap-3">
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => setFormTurno({ ...formTurno, documento: e.target.value })}
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => setFormTurno({ ...formTurno, nombrePaciente: e.target.value })}
                  ...
                  <div className="col-span-2 pt-1">
                    <label className="flex items-center space-x-2 text-xs font-semibold text-gray-700 cursor-pointer">
                      <input
                        ...
                        onChange={(e) => setFormTurno({ ...formTurno, requiereAcompanante: e.target.checked })}
                    ...
                    <div className="col-span-2 bg-blue-50/60 p-3 rounded-lg border border-blue-100 grid grid-cols-2 gap-2">
                      <div>
                        ...
                        <input
                          ...
                          onChange={(e) => setFormTurno({ ...formTurno, nombreAcompanante: e.target.value })}
                      ...
                      <div>
                        ...
                        <input
                          ...
                          onChange={(e) => setFormTurno({ ...formTurno, documentoAcompanante: e.target.value })}
                          className="w-full border border-gray-300 rounded px-2 py-1 text-xs bg-white"
                        />
                      </div>
                    </div>
                  ...
                  <div className="col-span-2 pt-2">
                    ...
                    >
                      🎟️ Asignar Turno e Ingresar a Sala
                    </button>
                  </div>
                </form>
              </div>
                    ...
                    <tbody className="divide-y text-gray-700">
                      {pacientesSala.map((p) => (
                        <tr key={p.id} className="hover:bg-gray-50/80">
                          <td className="p-2 font-mono text-[11px] text-gray-500">{p.horaIngreso}</td>
                          <td className="p-2 font-semibold text-gray-800">{p.nombre}</td>
                          <td className="p-2 font-mono">{p.documento}</td>
                          <td className="p-2 text-gray-600">{p.servicio}</td>
                          <td className="p-2">
                            <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold ${
                              p.estado === 'EN_ESPERA' ? 'bg-yellow-100 text-yellow-800' :
                              p.estado === 'AUSENTE' ? 'bg-red-100 text-red-800' : 'bg-green-100 text-green-800'
                            }`}>
                              {p.estado}
                            </span>
                          </td>
            ...
            {/* Lado Derecho (35%): Shell para Rentarhosting */}
            <div className="w-[35%] bg-white rounded-xl shadow-sm border border-gray-200 flex flex-col overflow-hidden">
              <div className="bg-gray-100 p-2.5 border-b border-gray-200 flex justify-between items-center">
                <span className="text-xs font-bold text-gray-700 truncate">🌐 Rentarhosting SAS</span>
                <span className="text-[10px] bg-emerald-100 text-emerald-800 font-bold px-1.5 py-0.5 rounded">
                  HC Externa
                </span>
              </div>
              <div className="flex-1 bg-gray-50 flex flex-col items-center justify-center p-4 text-center border-2 border-dashed border-gray-200 m-2 rounded-lg">
                <p className="text-xs font-bold text-gray-600 mb-1">Visor de Rentarhosting</p>
                <p className="text-[11px] text-gray-400 max-w-xs">
                  Aquí cargas la URL externa para verificar si el paciente existe o registrar la historia clínica.
                </p>
              </div>
            </div>
          ...
          <div className="flex-1 flex p-3 gap-3 overflow-hidden">
            <div className="w-[35%] bg-white rounded-xl shadow-sm border border-gray-200 p-3 flex flex-col">
              <h3 className="font-bold text-xs text-gray-700 uppercase mb-2 border-b pb-1">
                📋 Listado de Pacientes (Copiar ID)
              </h3>
              <div className="flex-1 overflow-y-auto space-y-1.5">
                {pacientesSala.map((p) => (
                  <div key={p.id} className="p-2 border rounded-lg flex justify-between items-center bg-gray-50 hover:bg-blue-50/50 transition">
                    <div>
                      <p className="font-bold text-xs text-gray-800">{p.nombre}</p>
                      <p className="text-[10px] text-gray-500 font-mono">CC: {p.documento}</p>
                    </div>
                    <button
                      onClick={() => copiarPortapapeles(p.documento)}
                      className="bg-[#1b75bb] text-white hover:bg-blue-700 text-[10px] px-2 py-1 rounded font-bold shadow-sm"
                    >
                      Copiar CC
                    </button>
                  </div>
            ...
            <div className="w-[65%] bg-white rounded-xl shadow-sm border border-gray-200 flex flex-col overflow-hidden">
              <div className="bg-gray-100 p-2.5 border-b text-xs font-bold text-gray-700">
                🧾 Módulo de Facturación Electrónica y RIPS (Rentarhosting SAS)
              </div>
              <div className="flex-1 flex items-center justify-center p-6 text-center text-gray-400 bg-gray-50">
                [ Visor Integrado de Facturación Externa ]
              </div>
            </div>
          </div>
        )}
          ...
          <div className="flex-1 p-4 overflow-y-auto space-y-4">
            <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-200">
              ...
              <div className="flex items-center space-x-3 bg-gray-50 p-2.5 rounded-lg border border-gray-200">
                <span className="text-xs font-semibold text-gray-600">Estado de Pantalla:</span>
                ...
                <tbody className="divide-y text-gray-700">
                  {pacientesSala.map((p) => (
                    <tr key={p.id} className="hover:bg-gray-50">
                      ...
                      <td className="p-2.5 text-gray-500 text-[11px] font-mono">{p.horaIngreso}</td>
                      <td className="p-2.5">
                        <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold ${
                          p.estado === 'EN_ESPERA' ? 'bg-yellow-100 text-yellow-800' :
                          p.estado === 'AUSENTE' ? 'bg-red-100 text-red-800' : 'bg-green-100 text-green-800'
